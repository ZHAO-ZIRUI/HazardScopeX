...
class FactorCaseDropObstical(Factor):
    NAME = 'F_CaseDropObstical'
    ...
    def create_act_vehicle(self) -> None:
        self._act = self._context.actors.create_vehicle(
            ...
            tf=self._context.spawn_points[self.MAPPING_WORLD_LOCATION[self._context.map_name][Factor.K_ACT]],
            name='ACT',
        )
    ...
    def create_obstacle_on_act_reach_obstacle_spawn_point(self) -> None:
        ...
        obstacle_tf = self._context.spawn_points[self.MAPPING_WORLD_LOCATION[self._context.map_name][Factor.K_OBSTACLE]]

...
class SemanticKittiDumper(DatasetDumper):
    ...
    def _flush_readme(self) -> None:
        ...
        data = StringIO()
        data.write('='*80 + '\n')
        data.write('THIS FILE IS AUTO-GENERATED BY HAREZED SCOPE X PROJECT\n')
        data.write('='*80 + '\n')
        data.write('\n')
        data.write('Dataset Class: ' + self.DATASET_CLASS + '\n')
        data.write('Dataset Name: ' + self._name + '\n')
        data.write('-'*20 + '\n')
        data.write('SENSOR NAME / FOLDER -> CALIBRATION MATRIX NAME\n')
        data.write('\n')

        # MAIN CAMERA
        data.write(self._main_camera.name + '[MAIN CAMERA] -> ' + 'P0' + '\n')
        # MAIN LIDAR
        data.write(self._main_lidar.name + '[MAIN LIDAR] -> ' + 'Tr' + '\n')
        # OTHER CAMERAS
        for idx, cam in enumerate(self._other_cameras, start=1):
            data.write(cam.name + '-> ' + f'P{idx}' + '\n')

...
class FactorCaseLongCargo(Factor):
    NAME = 'F_CaseLongCargo'
    ...
    def __init__(self, context: CarlaContext, ego: CarlaVehicle, *, distance_offset: float = 15.0, cargo_offset_behind: float = 0.2):
        super().__init__(context)
        self._ego = ego
        self._act: CarlaVehicle | None = None
        self._distance_offset = distance_offset
        self._cargo_offset_behind = cargo_offset_behind
        self._cargo: carla.Actor | None = None
    ...
    def tick(self) -> None:
        ...
        if (self._act is not None and self._act.actor is not None and self._act.actor.is_alive and
            ...
            # 销毁旧的cargo
            if self._cargo is not None and self._cargo.is_alive:
                self._cargo.destroy()
            ...
            self._cargo = self._context.world.spawn_actor(
                self._cargo_bp,
                cargo_transform
            )
            # 禁用物理模拟
            self._cargo.set_simulate_physics(False)

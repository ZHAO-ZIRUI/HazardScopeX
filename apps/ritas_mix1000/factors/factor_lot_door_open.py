...
class FactorLotDoorOpen(Factor):
    NAME = 'F_LotDoorOpen'
    ...
    def setup(self) -> None:
        ...
        for i in range(1, self._static_vehicle_count + 1):
            ...
            if s_vehicle.actor is not None and s_vehicle.actor.is_alive:
                ...
                if i == 4:
                    self._act = s_vehicle
                    self.debug.draw_point(current_location,size=0.3,color=carla.Color(0,255,0),life_time=1000)
    ...
    def tick(self) -> None:
        ...
        if (not self._act_triggered and not self._act_ap_enabled and
            ...
            if distance <= self._trigger_distance:
                self._act.actor.open_door(carla.VehicleDoor.All)
                print("______________________")
                self.logger.info(f'ACT Open the door...')

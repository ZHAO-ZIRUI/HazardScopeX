...
class ROS2HighPerformanceAdapter(IOAdapter):
    """
    ROS2 高性能适配器, 用于将仿真器的数据转换为 ROS2 数据

    高性能适配器将使用共享内存和多进程的方式来实现 ROS2 信息的发布和订阅, 以避免因 Python GIL 锁导致的性能瓶颈
    """

    DEFAULT_ROS_NODE_NAME = "HarzedScopeROS2HPNode"
    ...
    def __init__(
        ...
        self._shm_topic = shm_adapter.shm.name
